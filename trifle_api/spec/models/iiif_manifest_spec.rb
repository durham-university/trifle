...
RSpec.describe Trifle::API::IIIFManifest do
  ...
  let( :all_json_s ) {
    %q|{"resources":[
      {"id":"tajd472w44j","title":"Test title","digitisation_note":"test digitisation note","image_container_location":"testimages","identifier":["ark:/12345/tajd472w44j"],"date_published":"Xth century","author":["various authors"],"description":"test description","licence":"All rights reserved","attribution":"part of test items"},
  ...
  describe ".all" do
    it "parses the response" do
      expect(Trifle::API::IIIFManifest).to receive(:get).with('/manifest.json?per_page=1000').and_return(OpenStruct.new(body: all_json_s, code: 200))
  ...
  describe ".all_in_collection" do
    it "parses the response" do
      expect(Trifle::API::IIIFManifest).to receive(:get).with("/collection/#{collection.id}.json?full_manifest_list=1").and_return(OpenStruct.new(body: all_json_s, code: 200))
