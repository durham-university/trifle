...
RSpec.describe Trifle::IIIFAnnotationsController, type: :controller do
  ...
  let(:annotation_list) { FactoryGirl.create(:iiifannotationlist,:with_image) }
  let(:image) { FactoryGirl.create(:iiifimage) }
  ...
  context "with admin user" do
    ...
    describe "PUT #update" do
      ...
      it "returns iiif when requested" do
        put :update, reply_iiif: 'true', id: annotation.id, iiif_annotation: annotation_params
    ...
    describe "POST #create" do
      it "returns iiif when requested" do
        post :create, reply_iiif: 'true', iiif_annotation: annotation_params, iiif_annotation_list_id: annotation_list.id
        json = JSON.parse(response.body)
        expect(json['@type']).to eql('oa:Annotation')
        expect(json['on']).to be_present
        expect(json['resource']).to be_present
      ...
      context "with annotation_list" do
        it "adds to the annotation list" do
          ...
          post :create, iiif_annotation: annotation_params, iiif_annotation_list_id: annotation_list.id
      ...
      context "with image" do
        it "creates a new annotation list if needed and adds to it" do
          ...
          post :create, iiif_annotation: annotation_params, iiif_image_id: image.id
          image.reload
